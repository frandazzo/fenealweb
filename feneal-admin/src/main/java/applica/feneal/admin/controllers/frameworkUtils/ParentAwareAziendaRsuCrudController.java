...
/**
 * Created by felicegramegna 15/03/2021.
 */
@Controller
@RequestMapping("/parentaziendarsucrud")
public class ParentAwareAziendaRsuCrudController extends LocalizedController {
    ...
    public SimpleResponse grid(@PathVariable("entity") String entity, @PathVariable("firmId") Long firmId, String loadRequest) {
        ...
        try {
            ...
            lavIdFilter.setProperty("aziendaRsu");
            request.getFilters().add(lavIdFilter);

            Role r = ((Role) sec.getLoggedUser().getRoles().get(0));
            User u = ((User) sec.getLoggedUser());
            if (r.getLid() == 3){
                Filter companyFilter = new Filter();
                companyFilter.setType(Filter.EQ);
                companyFilter.setValue(u.getCompany().getLid());
                companyFilter.setProperty("companyId");
                request.getFilters().add(companyFilter);
            }

            if(r.getLid() == 1)
                request.setDisableOwnershipQuery(true);
    ...
    public @ResponseBody SimpleResponse form(@PathVariable("entity") String entity,@PathVariable("firmId") Long firmId, String id) {
        if(crudGuard != null) {
            ...
            try {
                String crudPermission =CrudPermission.EDIT;
        ...
        try {
            ...
            AziendaRSU aziendaRSU = azRsuRep.get(firmId).orElse(null);
            form.getData().put("aziendaRsu",aziendaRSU.getLid());
