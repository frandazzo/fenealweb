 ...
 */
@Controller
public class QuoteManualiController {
    ...
    @RequestMapping(value = "/quotemanuali",method = RequestMethod.GET)
    @PreAuthorize("isAuthenticated()")
    public @ResponseBody
    SimpleResponse view1(HttpServletRequest request) {
        try{
            Form form = new Form();
            form.setRenderer(applicationContext.getBean(QuoteImpiantiFormRenderer.class));
            form.setIdentifier("quotemanuali");
...
//                    .putParam(Params.FORM_COLUMN, "  ");
            ...
            response.setTitle("Creazione quote manuali");
    ...
    @RequestMapping(value="/quotemanuali/proceed", method = RequestMethod.POST)
    @PreAuthorize("isAuthenticated()")
    public @ResponseBody
    SimpleResponse doQuotemanuali(@RequestBody UiQuoteHeaderParams params){

        try{
            return new ValueResponse(quoteFacade.createRiepilogoQuoteManuali(params));
        }catch(Exception ex){
            ex.printStackTrace();
