...
define([
    ...
    var RepositoryServiceFactory = core.AObject.extend({
        ...
        searchDeleghe: function(searchParams){
            var route = BASE + "magazzinodeleghe/report" ;
    ...
    var MagazzinoDelegheAppView = fviews.FormAppView.extend({
        ctor: function(formService) {
            MagazzinoDelegheAppView.super.ctor.call(this, formService);
            ...
            self.formView.on("load", function(){
                ...
                $(".panel.col-div").css("height", "280px");
                ...
                //inserisco il codice per la creazione di un button
                var button = $(self.form.element).find('button[data-component="submit_button"]');
                var newButton = $('<button type="button" class="btn btn-secondary mrn">Aggiungi deleghe</button>');
                newButton.click(function(){

                    var formService = new fmodel.FormService();
                    formService.set("method", "GET");
                    formService.set("data", {});
                    formService.set("url", BASE + "magazzinodeleghe/add");

                    var container = $('<div class="magazzinodeleghe-add-container"></div>');

                    var formView = new fviews.FormView(formService);
                    formView.container = container;

                    formView.on("render", function() {
                        $(".magazzinodeleghe-add-container").find(".panel-footer, .panel-heading").hide();
                        ...
                        buttons: {
                            OK: {
                                ...
                                command: function() {
                                    ...
                                    var workerId = $(".magazzinodeleghe-add-container").find("input[name=worker]").val();

                                    if (!workerId)
                                        return;

                                    var province = $('.magazzinodeleghe-add-container').find('select[name="province"]').val();

                                    var tipoEnte = $('.magazzinodeleghe-add-container').find('select[name="tipoEnte"]').val();


                                //    alert(province + " " + tipoEnte);
                                    ...
                                    svc.set("url", BASE + "magazzinodeleghe/add");
                                    svc.set("contentType", "application/json")
                                    svc.set("data", JSON.stringify(magazzino));
                                    svc.set("method", "POST");
                ...
                svc.on("load", function(response){
                    ...
                    if ($(".generate-del").length == 0) {
                        ...
                        $(".generate-del").click(function() {
                            ...
                            location.href = BASE + "magazzinodeleghe/generadeleghe?province="+data.province+"&parithetic="+data.parithetic+"&collaborator="+data.collaborator;
                        });
                    }
        ...
        initGrid : function(responseData){
            ...
            var grid = $('#reportContainer').dxDataGrid({
                ...
                columns:[
                    ...
                    {dataField: "otherparithetics", caption:"Altri enti", visible : true, visibleIndex: 6},
    ...
    exports.MagazzinoDelegheAppView = MagazzinoDelegheAppView;
