...
define([
    ...
    var RepositoryServiceFactory = core.AObject.extend({
        ...
        searchDeleghe: function(searchParams){
            var route = BASE + "inoltradeleghe/report" ;
    ...
    var InoltraDelegheAppView = fviews.FormAppView.extend({
        ctor: function(formService) {
            InoltraDelegheAppView.super.ctor.call(this, formService);
            ...
            self.formView.on("load", function(){
                ...
                $(".panel.col-div").css("height", "270px");
            ...
            self.formView.on("submit", function(){
                ...
                svc.on("load", function(response){
                    ...
                    // aggiungo tasto INOLTRA DELEGHE
                    if ($(".send-del").length == 0) {

                        var delSend = '<div class="col-md-12 col-xs-3 margin-bottom-10 p0" data-toggle="tooltip" data-placement="top" title="Inoltra deleghe">' +
                            '<button type="button" class="btn btn-primary full-width send-del">' +
                            '<span class="glyphicon glyphicon-open" aria-hidden="true"></span>' +
                            '</button></div>';

                        $(".toolbox-buttons-cnt").append($(delSend));
                        $(".send-del").parent().tooltip();
                        $(".send-del").click(function() {

                            if (grid.getSelectedRowsData().length == 0) {
                                $.notify.error("Selezionare almeno una delega");
                                return false;
                            }

                            var container = $('<div class="send-del-container"></div>');

                            var path = BASE + "deleghe/inoltra";
                                ...
                                buttons: {
                                    Salva: {
                                        ...
                                        command: function() {
                                            ...
                                            data.selectedDeleghe = selectedrows;

                                            var svc = new fmodel.AjaxService();
                                            svc.url = BASE + "deleghe/inoltra";
                                            svc.set("method", "POST");
                    ...
                    var reportResultsConfigurer = new resultsConfigurer.ReportUiConfigurer(grid, "inoltra deleghe", false);
                ...
                columns:[
                    ...
                    { dataField:"lavoratoreNomeCompleto", fixed :true, visible : true, visibleIndex: 0,
                        cellTemplate: function (container, options) {
                            ...
                            var province = options.data.delegaProvincia;
    ...
    exports.InoltraDelegheAppView = InoltraDelegheAppView;
