 ...
 */
@Component
public class IscrizioneInefficaceImporter implements IImportDelegaBari {
    ...
    public void importDelega(Lavoratore l, ImportDelegheSummaryForBari summary, File f) throws Exception {
        ...
        delSvc.writeToFile(f, "*******Entro in IscrizioneInefficaceImporter per il lavoratore " + l.getSurnamename() );
        ...
        LoadRequest req1 = LoadRequest.build().filter("description", summary.getSubscription());
        ...
        if (d.size()  == 0){
            delSvc.writeToFile(f, "*******Non ho trovato deleghe in data protocollo. procedo alla creazione e revoca inefficace" );
            ...
            //revoco tutte le precedenti adesso
            List<Delega> dn = delSvc.getAllWorkerDeleghe(l.getLid())
                    .stream()
        ...
        //le revoco tutte nella stessa data
